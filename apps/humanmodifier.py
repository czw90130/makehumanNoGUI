...
class ManagedTargetModifier(Modifier):
    ...
    def findTargets(path):
        ...
        检索指定目标路径下分组的目标列表（这不是直接的文件系统路径，而是一个抽象，路径是
        由一个符号分隔的层次原子字符串）。
        
        结果是元组列表，每个元组为：
        （目标路径，事实依赖性），
        带有目标路径引用.target文件的filepath，而事实依赖于列表，列表具有变量的名称或影
        响权重的因素，并施加了目标文件的数量。由此产生的权重。应用目标是在事实依赖性列表
        中声明的所有因子值的乘法。
        
        这些事实依赖性中的一些来自预定的宏参数（例如年龄，种族，种族，重量，性别，性别，...），
        目标模块已经提供从目标路径或文件名提取已知参数。
        可以随意添加其他事实依赖性，以直接控制目标的加权（例如，将此修饰符的值应用于其目标）。
        将修饰符值设置为最终设置目标重量（宏修饰符使用）的因素的另一种方法是确保通过在人类对
        象上调用相应的设置器来确保更新人类对象的XVAL变量。默认情况下，getFactors（）将解析
        已知XVAL变量的值为已知因子变量名称。
        
        事实依赖性可以是任何名称，并且不仅限于目标路径中发生的令牌。尽管对于每个事实依赖性，
        但GetFactors（）必须返回匹配值。
        """
        if path is None:
            return []
...
class MacroModifier(ManagedTargetModifier):
    ...
    更复杂的修饰符控制许多目标，由目标模块管理。宏修饰符不直接影响目标权重，而是控制宏变
    量对人类的值，这些值决定了一组宏目标的最终组合。因为宏修饰符不能直接控制目标，所以可
    能有许多宏修饰符控制同一组目标，但通过影响不同的宏观变量来实现
    """
...
class EthnicModifier(MacroModifier):
    ...
    宏修改器的专业化管理三个紧密连接的修饰符，其总价值总和必须总和为1。
    """
    def __init__(self, groupName, variable):
